...
export const VOICE_TOOL_DECLARATIONS = [
  {
    name: "create_tasks",
    description:
      "Create one or more new tasks/quests. Use when the user describes tasks to add.",
    parameters: {
      type: "OBJECT",
      properties: {
        tasks: {
          ...
          items: {
            ...
            properties: {
              ...
              startTime: {
                type: "STRING",
                description:
                  "Specific start time as ISO datetime. Only when user specifies a time.",
              },
              blockType: {
                type: "STRING",
                enum: ["focus", "admin", "personal", "meeting", "class"],
                description: "Time block category",
              },
            },
  ...
  },
  {
    ...
    description:
      "Auto-schedule all unscheduled tasks into available time blocks. Use when the user asks to plan or schedule everything.",
    parameters: {
      type: "OBJECT",
      properties: {
        startAfter: {
          type: "STRING",
          description:
            "Earliest allowed start time as ISO datetime.",
        },
      },
    },
  },
  {
    name: "confirm_plan",
    description:
      "Commit the proposed schedule to the calendar. Use when user says 'confirm', 'looks good', 'do it'.",
    parameters: {
      type: "OBJECT",
      properties: {},
    },
  },
  {
    name: "adjust_block",
    description:
      "Move or reschedule an existing task to a specific time.",
    parameters: {
      type: "OBJECT",
      properties: {
        blockTitle: {
          type: "STRING",
          description: "The task/block name to adjust",
        },
        change: {
          type: "STRING",
        ...
        newEstimateMinutes: {
          type: "NUMBER",
          description: "New duration in minutes if changed",
        },
        newStartTime: {
          type: "STRING",
          description: "New start time as ISO datetime",
        },
        startAfter: {
          type: "STRING",
          description: "Earliest allowed start time if rescheduling",
        },
      },
...
/**
 * Execute a tool call from Gemini Live API using the existing action handler.
 * Returns the result and a human-readable summary for the voice toast.
 */
...
/**
 * Compute the current week range (Monday to Sunday) for the given timezone.
 */
